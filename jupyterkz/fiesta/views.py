...
def index(request, folder_name=""):
    ...
    if folder_name == "":
        files = []
        dirs = [{"folder": i.username, "name": i.first_name, "visits": sum([html.visits for html in Html.objects.filter(author_id=i.id)])} for i in User.objects.all() if not i.is_superuser]
    else:
...
def sitemap(request):
    urls = ["http://jupyter.kz/"+ i.username for i in User.objects.all() if not i.is_superuser]
    urls += ["http://jupyter.kz" + i.attachment.attachment.url for i in Html.objects.all()]
